...
public record LodestoneTeleportationCheck(@Nullable Component failureComponent, Predicate<LodestoneTrackerContext> shouldProceed) {
    ...
    public static final LodestoneTeleportationCheck IS_WITHIN_LODESTONE_RANGE = new LodestoneTeleportationCheck(Component.translatable("item.enderscape.lodestone_teleportation.message.too_far"), LodestoneTeleportation::isWithinRange);
    ...
    public static final LodestoneTeleportationCheck IS_WITHIN_SIZE_LIMITS = new LodestoneTeleportationCheck(Component.translatable("item.enderscape.lodestone_teleportation.message.unsafe"), LodestoneTeleportationCheck::isWithinSizeLimits);
    ...
    public static final List<LodestoneTeleportationCheck> CHECKS_IN_ORDER = List.of(
            ...
            IS_WITHIN_LODESTONE_RANGE,
            FUEL_EXCEEDS_COST,
            IS_WITHIN_SIZE_LIMITS,
    ...
    private static boolean isWithinSizeLimits(LodestoneTrackerContext context) {
        ItemStack stack = context.stack();

        if (LodestoneTeleportation.is(stack)) {
            LivingEntity user = context.user();
            EntityDimensions dimensions = user.getDimensions(user.getPose());
            Vec2 maximum = LodestoneTeleportation.get(stack).rules().maximumEntitySize();
