...
public record LodestoneTeleportationVisuals(
    ...
    private static final ParticleOptions DEFAULT_TELEPORT_OUT_PARTICLE = EnderscapeParticles.MIRROR_TELEPORT_OUT;
    private static final ParticleOptions DEFAULT_TELEPORT_IN_PARTICLE = EnderscapeParticles.MIRROR_TELEPORT_IN;
    private static final AssetInfo DEFAULT_OVERLAY_TEXTURE = assetOf(Enderscape.id("misc/overlay"));
    private static final AssetInfo DEFAULT_VIGNETTE_TEXTURE = assetOf(Enderscape.id("misc/vignette"));
    ...
    public record AssetInfo(ClientAsset.ResourceTexture asset) {
        public static final Codec<AssetInfo> CODEC = RecordCodecBuilder.create(instance -> instance.group(ClientAsset.ResourceTexture.CODEC.fieldOf("asset").forGetter(AssetInfo::asset)).apply(instance, AssetInfo::new));
        public static final StreamCodec<ByteBuf, AssetInfo> STREAM_CODEC = ClientAsset.ResourceTexture.STREAM_CODEC.map(AssetInfo::new, AssetInfo::asset);
    }

    private static AssetInfo assetOf(Identifier id) {
        return new AssetInfo(new ClientAsset.ResourceTexture(id));
    }
}