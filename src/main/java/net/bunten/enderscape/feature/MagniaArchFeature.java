                ...
                for (int y = -radius; y <= radius; y++) {
                    for (int z = -radius; z <= radius; z++) {
                        ...
                        if (x * x + y * y + z * z <= UniformInt.of(radius, radius * radius).sample(random) && level.getBlockState(pos).canBeReplaced()) {
                            level.setBlock(pos, EnderscapeBlocks.ALLURING_MAGNIA.defaultBlockState(), radius);
                            decorations.add(pos);
    ...
    private void placeMagniaSprouts(Set<BlockPos> decorations, RandomSource random, WorldGenLevel level) {
        decorations.forEach(pos -> {
            for (Direction dir : Direction.values()) {
                ...
                if (random.nextInt(15) == 0 && level.getBlockState(pos).is(EnderscapeBlocks.ALLURING_MAGNIA) && level.getBlockState(relative).canBeReplaced() && level.getBlockState(pos.relative(dir, 2)).canBeReplaced()) {
                    if (random.nextInt(200) == 0) {
                        ...
                        level.setBlock(relative, EnderscapeBlocks.ALLURING_MAGNIA_SPROUT.defaultBlockState().setValue(MagniaSproutBlock.FACING, dir), 2);
                    }
                }
            }
        });
