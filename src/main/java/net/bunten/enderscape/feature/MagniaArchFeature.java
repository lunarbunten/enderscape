    ...
    private void placeMagniaSprouts(Set<BlockPos> decorations, RandomSource random, WorldGenLevel level) {
        decorations.forEach(pos -> {
            for (Direction dir : Direction.values()) {
                if (random.nextInt(15) == 0 && level.getBlockState(pos).is(EnderscapeBlocks.ALLURING_MAGNIA) && level.getBlockState(pos.relative(dir)).canBeReplaced()) {
                    level.setBlock(pos.relative(dir), EnderscapeBlocks.ALLURING_MAGNIA_SPROUT.defaultBlockState().setValue(MagniaSproutBlock.FACING, dir), 2);
                }
            }
        });
    }
