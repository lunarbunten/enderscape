...
public class LargeMurublightChanterelleGenerator {
    ...
    public static boolean generate(LevelAccessor level, BlockPos origin, RandomSource random, LargeMurublightChanterelleConfig config, int height) {
        ...
        if (level.getBlockState(origin.relative(direction.getOpposite())).is(EnderscapeBlocks.CORRUPT_OVERGROWTH)) {
            place(level, origin.relative(direction.getOpposite()), EnderscapeBlocks.MIRESTONE.defaultBlockState());
        }
        ...
        for (int a = 0; a < height; a++) place(level, origin.relative(direction, a), EnderscapeBlocks.MURUBLIGHT_STEM.defaultBlockState().setValue(RotatedPillarBlock.AXIS, direction.getAxis()));
