...
public class DrifterRenderer extends MobRenderer<Drifter, DrifterModel> {
    ...
    public DrifterRenderer(EntityRendererProvider.Context context) {
        super(context, new DrifterModel(context.bakeLayer(EnderscapeModelLayers.DRIFTER)), 1.0F);
        this.adultModel = model;
        this.babyModel = new DrifterModel(context.bakeLayer(EnderscapeModelLayers.DRIFTLET));
