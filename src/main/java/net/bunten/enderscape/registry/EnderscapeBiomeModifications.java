...
public class EnderscapeBiomeModifications {
    ...
    private static void registerAmbienceModifications() {
        conditionalAmbienceModification(
                ...
                selection -> selection.getBiome().getBackgroundMusic().isPresent() && selection.getBiome().getBackgroundMusic().get().equals(Musics.END),
    ...
    private static void registerFeatureModifications() {
        ...
        BiomeModifications.create(Enderscape.id("add_new_barrens_content")).add(
                ...
                (selection, modification) -> {
                    modification.getSpawnSettings().addSpawn(MobCategory.MONSTER, new MobSpawnSettings.SpawnerData(EnderscapeEntities.RUBBLEMITE, 2, 2, 3));
