...
public class EnderscapeVaultLootProvider implements LootTableSubProvider {
    private final HolderLookup.Provider lookup;
    public EnderscapeVaultLootProvider(HolderLookup.Provider lookup) {
        this.lookup = lookup;
    }
    ...
    public LootTable.Builder endCityVaultRewards() {
        try {
            ...
            EnchantWithLevelsFunction.Builder toolEnchantments = EnchantWithLevelsFunction.enchantWithLevels(lookup, UniformGenerator.between(20, 40));
            ...
            return LootTable.lootTable()
                    ...
                    .withPool(
                            LootPool.lootPool()
                                    ...
                                    .add(createItem(MUSIC_DISC_GLARE.get(), 3))
                                    .add(createItem(CRESCENT_BANNER_PATTERN.get(), 3))
                                    .add(createItem(STASIS_ARMOR_TRIM_SMITHING_TEMPLATE.get(), 3))
                    )
                    .withPool(
                            LootPool.lootPool()
