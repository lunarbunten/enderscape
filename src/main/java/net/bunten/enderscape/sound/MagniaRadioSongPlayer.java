...
public class MagniaRadioSongPlayer {
    ...
    public void play(LevelAccessor level, Holder<MagniaRadioSong> holder) {
        ...
        int value = level.registryAccess().registryOrThrow(EnderscapeRegistries.MAGNIA_RADIO_SONG).getId(song.value());
    ...
    public void stop(LevelAccessor level, @Nullable BlockState state, boolean updateBlock) {
        if (song != null) {
            ...
            if (state != null && updateBlock) level.setBlock(pos, state.setValue(StateProperties.IS_PLAYING, false), 2);
            onChanged.notifyChange();
        }
    }
