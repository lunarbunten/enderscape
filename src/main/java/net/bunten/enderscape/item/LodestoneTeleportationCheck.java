...
public record LodestoneTeleportationCheck(@Nullable Component failureComponent, Predicate<LodestoneTrackerContext> shouldProceed) {
    ...
    public static final LodestoneTeleportationCheck IS_LINKED_TO_LODESTONE = new LodestoneTeleportationCheck(Component.translatable("item.enderscape.lodestone_teleportation.message.unlinked"), (context) -> LodestoneTeleporter.isLinked(context.stack()));
    ...
    public static final LodestoneTeleportationCheck IS_SAME_COORDINATE_SCALE = new LodestoneTeleportationCheck(Component.translatable("item.enderscape.lodestone_teleportation.message.wrong_coordinate_scale"), LodestoneTeleporter::isSameCoordinateScale);
    ...
    public static final LodestoneTeleportationCheck TELEPORT_POSITION_IS_SAFE = new LodestoneTeleportationCheck(Component.translatable("item.enderscape.lodestone_teleportation.message.unsafe"), context -> LodestoneTeleporter.getTeleportPosition(context).isPresent());
    ...
    public InteractionResultHolder<ItemStack> getFailureResult(LodestoneTrackerContext context) {
        return failureComponent != null ? failureResult(context, Component.translatable(failureComponent.getString()).withStyle(ChatFormatting.RED)) : InteractionResultHolder.consume(context.stack());
    }

    private static InteractionResultHolder<ItemStack> failureResult(LodestoneTrackerContext context, Component failureComponent) {
    ...
    private static boolean canTravelTo(LodestoneTrackerContext context) {
        return LodestoneTeleporter.isTransdimensionalAllowed(context) || LodestoneTeleporter.isSameDimension(context, context.linkedDimension());
    }
