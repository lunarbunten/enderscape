...
void main() {
    ...
    if (UseBrightLightmap != 0) {
        color = mix(color, vec3(0.93, 1.1, 0.93), 0.01);
        color = clamp(color, 0.0, 1.0);
    ...
    float adjustedBrightnessFactor = UseBrightLightmap != 0 ? BrightnessFactor * 0.75 : BrightnessFactor;
    color = mix(color, notGamma, adjustedBrightnessFactor);
    color = mix(color, vec3(0.75), 0.04);
    color = clamp(color, 0.0, 1.0);
